...
"""
    ...
    def check_initialized(self):
        if not self.initialized:
            self.initialize()
	"""
	函数是一个自底向上的动态规划问题，它从sentence的最后一个字（N-1）开始倒序遍历sentence的每个字（idx）的方式，计算子句sentence[idx ~ N-1]的概率对数得分。
    ...
    def __cut_DAG_NO_HMM(self, sentence):
        ...
        while x < N:
            ...
            if re_eng.match(l_word) and len(l_word) == 1:
                #如果l_word是单个的字母或是数字，则希望将它与下一个词连起来
                buf += l_word
                x = y
            else:
                if buf:
